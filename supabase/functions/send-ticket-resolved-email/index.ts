...
Deno.serve(async (req) => {
  ...
  try {
    const { ticketId, email, clientName, issueTitle, priority, issueCategory, department, resolvedBy, chatLink: providedChatLink, orgName } = await req.json()
...
<body style="margin: 0; padding: 0; background: #f0f2f5; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
  <div style="max-width: 640px; margin: 0 auto; background: #ffffff; border-radius: 12px; overflow: hidden; margin-top: 24px; margin-bottom: 24px; box-shadow: 0 4px 24px rgba(0,0,0,0.08);">
    ...
    <!-- Header -->
    <div style="background: linear-gradient(135deg, #065f46 0%, #047857 50%, #059669 100%); padding: 40px 32px; text-align: center;">
      <div style="font-size: 48px; margin: 0 auto 16px; text-align: center;">✅</div>
      <h1 style="color: #ffffff; margin: 0; font-size: 26px; font-weight: 700; letter-spacing: -0.5px;">Ticket Resolved</h1>
      <div style="width: 48px; height: 3px; background: rgba(255,255,255,0.4); margin: 12px auto 0; border-radius: 2px;"></div>
      <p style="color: #a7f3d0; margin: 12px 0 0; font-size: 14px;">Your support issue has been addressed</p>
    </div>
    ...
    <div style="background: #f0fdf4; padding: 16px 32px; border-bottom: 1px solid #bbf7d0;">
      <table style="width: 100%;" cellpadding="0" cellspacing="0">
        <tr>
          <td>
            <span style="display: inline-block; background: #dcfce7; color: #15803d; padding: 6px 14px; border-radius: 20px; font-size: 12px; font-weight: 600; letter-spacing: 0.5px;">✅ CLOSED</span>
          </td>
    ...
    <div style="padding: 32px;">
      ...
      <p style="color: #4b5563; font-size: 15px; line-height: 1.7; margin: 0 0 24px;">
        Great news! Your support ticket <strong style="color: #059669;">#${shortId}</strong> has been resolved and closed. Here's a summary:
      </p>
      ...
      <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden; margin: 0 0 24px;">
        <div style="background: #f0fdf4; padding: 14px 20px; border-bottom: 1px solid #bbf7d0;">
          <h3 style="margin: 0; font-size: 14px; color: #166534; font-weight: 700; letter-spacing: 0.3px;">📋 RESOLVED TICKET SUMMARY</h3>
        </div>
          ...
          <table style="width: 100%; border-collapse: collapse;">
            ...
            <tr style="border-top: 1px solid #f1f5f9;">
              ...
              <td style="padding: 10px 0;">
                <span style="display: inline-block; background: #dcfce7; color: #15803d; padding: 3px 12px; border-radius: 12px; font-size: 12px; font-weight: 700;">Resolved</span>
              </td>
            </tr>
            ${resolvedBy ? `<tr style="border-top: 1px solid #f1f5f9;">
              <td style="padding: 10px 0; color: #6b7280; font-size: 13px;">Resolved By</td>
              <td style="padding: 10px 0; color: #1f2937; font-size: 13px; font-weight: 500;">${resolvedBy}</td>
            </tr>` : ''}
          </table>
        </div>
      </div>
      ...
      <!-- Need More Help -->
      <div style="background: #fefce8; border: 1px solid #fde68a; border-radius: 10px; padding: 20px; margin: 0 0 28px;">
        <h4 style="margin: 0 0 8px; color: #92400e; font-size: 14px; font-weight: 700;">💡 Still need help?</h4>
        <p style="margin: 0; color: #78350f; font-size: 13px; line-height: 1.6;">
          If the issue persists or you need further assistance, you can reply through the messages link below or submit a new ticket. We're always here to help!
        </p>
      </div>
      
      <!-- CTA Button -->
      <div style="text-align: center; margin: 0 0 24px;">
        <a href="${chatLink}" style="display: inline-block; background: linear-gradient(135deg, #059669 0%, #047857 100%); color: #ffffff; text-decoration: none; padding: 16px 40px; border-radius: 10px; font-size: 15px; font-weight: 700; letter-spacing: 0.3px; box-shadow: 0 4px 14px rgba(5, 150, 105, 0.4);">
          💬 View Ticket Messages
        </a>
      </div>
      
      <p style="color: #9ca3af; font-size: 12px; line-height: 1.6; margin: 0; text-align: center;">
        Thank you for using HigherView Taxes Support.
      </p>
    </div>
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: [email],
        subject: `✅ Ticket #${shortId} Resolved — ${issueTitle}`,
        html,
      }),
    })
    ...
    console.log('Resolution email sent via Resend:', result)
    ...
    console.error('Resolution email error:', errorMsg)
    return new Response(
